...
function tickRoom(roomId, r){
  ...
  // fechar mapa
  if(now - r.startedAt > 4000){
    r.close = Math.min(520, r.close + 0.45);
  }
  ...
  for(const e of r.enemies){
    ...
    e.x = clamp(e.x + (dx/d)*1.3, 18, MAP.w-18);
    e.y = clamp(e.y + (dy/d)*1.3, 18, MAP.h-18);
    ...
    if(d<420 && e.cd===0){
      e.cd=35;
      r.bullets.push({ x:e.x, y:e.y, vx:(dx/d)*7, vy:(dy/d)*7, from:"e", life:80 });
    }
  }
    ...
    if(!dead && b.from==="e"){
      for(const p of Object.values(r.players)){
        ...
        if(d2(b.x,b.y,p.x,p.y)<20*20){
          p.hp-=10; if(p.hp<=0){ p.hp=0; p.alive=false; }
          dead=true; break;
        }
      }
    }
  ...
  // saída → ganha quem matou mais
  if(alive.length>0){
    const inDoor = alive.filter(p=>d2(p.x,p.y,r.door.x,r.door.y)<(r.door.r+20)**2);
...
setInterval(()=>{ for(const [id,r] of ROOMS) tickRoom(id,r); }, TICK);
...
wss.on("connection",(ws)=>{
  ...
  ws.on("message",(raw)=>{
    ...
    if(m.t==="join"){
      ...
      r.players[id]={ x:rand(300,500), y:rand(300,500), ix:0, iy:0,
        aimX:1, aimY:0, hp:max, max, alive:true, cls,
        name:String(m.name||id).slice(0,14), kills:0 };
    ...
    if(m.t==="shoot" && p.alive){
      ...
      r.bullets.push({
        x:p.x+(p.aimX/len)*28, y:p.y+(p.aimY/len)*28,
        vx:(p.aimX/len)*9, vy:(p.aimY/len)*9,
        from:`p:${id}`, life:75
      });
    }

    if(m.t==="reset"){
      r.enemies=[]; r.bullets=[]; r.lifes=[];
      r.close=0; r.startedAt=Date.now(); r.result=null;
      for(const pp of Object.values(r.players)){
        pp.hp=pp.max; pp.alive=true; pp.kills=0;
        pp.x=rand(300,500); pp.y=rand(300,500);
      }
    }
  });
