...
function roomGet(roomId) {
  if (!ROOMS.has(roomId)) {
    ROOMS.set(roomId, {
      players: {}, // id -> {x,y,hp,max,alive,cls,aimX,aimY,name}
      clients: new Map(), // ws -> id
      bullets: [],
      enemies: [],
      lifes: [],
      close: 0,
      door: { x: MAP.w - 260, y: MAP.h - 220, r: 36 },
      startedAt: Date.now(),
      lastEnemyAt: 0,
      lastLifeAt: 0,
      msg: ""
...
function allAliveInDoor(r) {
  ...
  for (const id of ids) {
    ...
    if (dist2(p.x, p.y, r.door.x, r.door.y) > (r.door.r + 18) ** 2) return false;
  }
  return true;
}
...
function tickRoom(roomId, r) {
  ...
  for (const e of r.enemies) {
    ...
    if (d < 520 && e.cd === 0) {
      ...
      const vx = (dx / d) * 8.2;
      const vy = (dy / d) * 8.2;
      r.bullets.push({ x: e.x, y: e.y, vx, vy, from: "e", life: 90 });
    }
  }
  ...
  // win/lose conditions
  const ids = Object.keys(r.players);
  const everyoneConnected = ids.length >= 2;
  const someoneDead = Object.values(r.players).some(p => p && !p.alive);

  let result = null;
  if (everyoneConnected && someoneDead) result = "LOSE";
  if (everyoneConnected && allAliveInDoor(r)) result = "WIN";
  ...
  if (result) {
    // freeze a bit by stopping updates after result (optional)
    // Keep sending snapshots for a moment
  }
}
...
wss.on("connection", (ws) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.t === "join") {
      ...
      safeSend(ws, { t: "you", id: playerId, roomId });
    ...
    if (msg.t === "state") {
      // server trusts position (protótipo). Clamp to map.
      p.x = clamp(Number(msg.x) || p.x, 18, MAP.w - 18);
      p.y = clamp(Number(msg.y) || p.y, 18, MAP.h - 18);
